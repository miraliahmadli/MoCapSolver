...
class SkeletonConv(nn.Module):
    def __init__(self, neighbour_list, in_channels, out_channels, kernel_size, joint_num, stride=1, padding=0,
                 bias=True, padding_mode='zeros', add_offset=False, in_offset_channel=0, first_conv=False, last_conv=False):
        ...
        for neighbour in neighbour_list:
            ...
            if first_conv and (self.joint_num - 1 in neighbour or 0 in neighbour):
                idx = self.joint_num*self.in_channels_per_joint
                for i in range(self.in_channels_per_joint):
                    expanded.append(idx+i)
        ...
        for i, neighbour in enumerate(self.expanded_neighbour_list):
            ...
            if last_conv:
                neighbour_ = neighbour_list[i]
                if self.joint_num - 1 in neighbour_ or 0 in neighbour_:
                    self.mask[-4:, i, ...] = 1
    ...
    def forward(self, input):
        ...
        if self.add_offset and self.offset is not None:
